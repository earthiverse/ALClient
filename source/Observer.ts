...
export class Observer {
    ...
    public async connect(reconnect = false, start = true): Promise<void> {
        this.socket = socketio(`ws${this.serverData.secure ? "s" : ""}://${this.serverData.address}`, {
            autoConnect: false,
    ...
    protected async parseEval(data: EvalData): Promise<boolean> {
        if (typeof data === "string") {
            ...
            // TODO: We need to check when it was last updated, or we may decrease the time too much
            // const temporalSurgeReg = /^assassin_smoke\((-?\d+(?:\.\d+)?),(-?\d+(?:\.\d+)?),'icecrack/.exec(data)
            // if (temporalSurgeReg) {
