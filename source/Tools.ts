...
export class Tools {
     ...
     * @param b Position 2
     */
    public static squaredDistance(a: { x: number, y: number, width?: number, height?: number, map?: MapName, in?: string }, b: { x: number, y: number, width?: number, height?: number, map?: MapName, in?: string }): number {
        if (!a || !b) return Number.MAX_VALUE // No data for one of the objects
        if ((a.map && b.map) && (a.map !== b.map)) return Number.MAX_VALUE // Different map
        if ((a.in && b.in) && (a.in !== b.in)) return Number.MAX_VALUE // Different instance
